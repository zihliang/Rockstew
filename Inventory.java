...
{
    public List<Actor> collectedObjects = new ArrayList(0); //the items which are collected 
    ...
    { //create inventory
        ...
        GreenfootImage img = new GreenfootImage(320, 45); //size of the inventory
        img.setColor(new Color(100, 0, 0)); //color of the inventory background
        img.fill();
        background.setImage(img);
        getWorld().addObject(background, getWorld().getWidth()/2, getWorld().getHeight() - 25); //add it to the middle of the world
        ...
        {
            for (int column=0; column<10; column++) //each row has 5 columns
            {
                Actor slot = new Actor() {}; //draw the inventory slots on the image of the 'background' object
                slot.setImage(img);
                getWorld().addObject(slot, (background.getX() - background.getImage().getWidth()/2 + slot.getImage().getWidth()/2 + 10) + column*30, (background.getY() 
                - background.getImage().getHeight()/2 + slot.getImage().getHeight()/2 + 10) + row*30);
